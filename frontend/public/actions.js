...
(function () {
    const isActionsPage =
        location.pathname.endsWith("/actions.html");
    ...
    function loadAttacks(data) {
        for (let i = 1; i <= 5; i++) {
            setValue(
                `attack_${i}_type`,
                data[`attack_${i}_type`] ?? ""
            );

            setValue(
                `attack_${i}_abil`,
                data[`attack_${i}_abil`] ?? ""
            );

            setValue(
                `attack_${i}_prof`,
                data[`attack_${i}_prof`] ?? false
            );

            setValue(
                `attack_${i}_misc`,
                data[`attack_${i}_misc`] ?? 0
            );

            setValue(
                `attack_${i}_range`,
                data[`attack_${i}_range`] ?? ""
            );

            setValue(
                `attack_${i}_damage`,
                data[`attack_${i}_damage`] ?? ""
            );
        }
    }
    ...
    function recalcAttacks() {
        ...
        const proficiencyBonus =
            toNum(data.proficiency_bonus);

        for (let i = 1; i <= 5; i++) {
            const ability =
                getValue(`attack_${i}_abil`);

            const proficient =
                Boolean(getValue(`attack_${i}_prof`));

            const misc =
                toNum(getValue(`attack_${i}_misc`));

            let modifier = 0;

            if (ability) {
                const score =
                    toNum(data[ability] ?? 10);
    ...
    async function saveAttacks() {
        ...
        try {
            const latestCharacter =
                await API.getCharacter(currentCharacter.id);

            const attackData = {};

            for (let i = 1; i <= 5; i++) {
                attackData[`attack_${i}_type`] =
                    getValue(`attack_${i}_type`);

                attackData[`attack_${i}_abil`] =
                    getValue(`attack_${i}_abil`);

                attackData[`attack_${i}_prof`] =
                    Boolean(getValue(`attack_${i}_prof`));

                attackData[`attack_${i}_misc`] =
                    toNum(getValue(`attack_${i}_misc`));

                attackData[`attack_${i}_range`] =
                    getValue(`attack_${i}_range`);

                attackData[`attack_${i}_damage`] =
                    getValue(`attack_${i}_damage`);
            }
    ...
    function openActionEditor(action) {
        const editor =
            document.getElementById("actionEditor");
        ...
        const limited =
            document.getElementById("actionLimitedUses");

        const usesMax =
            document.getElementById("actionUsesMax");

        const resetOn =
            document.getElementById("actionResetOn");

        const usesOptions =
            document.getElementById("actionUsesOptions");

        const deleteButton =
            document.getElementById("btnDeleteAction");
        ...
        if (limited) {
            limited.checked =
                Boolean(action.limited_uses);
        }

        if (usesMax) {
            usesMax.value =
                String(action.uses_max ?? 1);
        }

        if (resetOn) {
            resetOn.value =
                action.reset_on ?? "manual";
        }
    ...
    function renderActions() {
        ...
        for (const action of actions) {
            ...
            if (action.limited_uses) {
                const usesMax =
                    Math.max(1, Number(action.uses_max) || 1);
        ...
        <div class="actionCard__uses">
            ...
            <small>
                Regeneration:
                ${escapeActionHtml(
                    resetLabels[action.reset_on] ?? "Manuell"
                )}
            </small>
        </div>
    `;
...
`;
            ...
            card
                ...
                .forEach((button) => {
                    button.addEventListener("click", (event) => {
                        ...
                        const index =
                            Number(button.dataset.useIndex);

                        console.log(
                            "[actions] Anwendung geklickt",
                            action.id,
                            index
                        );
    ...
    async function loadCharacter() {
        ...
        try {
            const character =
                await API.getCharacter(characterId);
            ...
            console.error(
                "[actions] Charakter konnte nicht geladen werden",
                error
            );
    ...
    function bindAttackCalculation() {
        for (let i = 1; i <= 5; i++) {
            const type =
                document.getElementById(`attack_${i}_type`);

            const ability =
                document.getElementById(`attack_${i}_abil`);

            const proficient =
                document.getElementById(`attack_${i}_prof`);

            const misc =
                document.getElementById(`attack_${i}_misc`);

            const range =
                document.getElementById(`attack_${i}_range`);

            const damage =
                document.getElementById(`attack_${i}_damage`);
    ...
    async function saveNewAction() {
        ...
        const limitedUses =
            document
                .getElementById("actionLimitedUses")
                ?.checked ?? false;

        const usesMaxRaw =
            Number(
                document
                    .getElementById("actionUsesMax")
                    ?.value
            );
        ...
        try {
            ...
            console.error(
                "[actions] Aktion konnte nicht gespeichert werden",
                error
            );
    ...
    async function deleteCurrentAction() {
        ...
        const confirmed =
            confirm("Diese Aktion wirklich löschen?");
        ...
        try {
            ...
            console.error(
                "[actions] Aktion konnte nicht gelöscht werden",
                error
            );
    ...
    function bindActionEditor() {
        const btnAdd =
            document.getElementById("btnAddAction");

        const btnCancel =
            document.getElementById("btnCancelAction");

        const btnSave =
            document.getElementById("btnSaveAction");
